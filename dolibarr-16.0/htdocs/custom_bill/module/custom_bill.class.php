 ...
 */
class modPocTest extends DolibarrModules
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->numero = 428401; // TODO Go on page https://wiki.dolibarr.org/index.php/List_of_modules_id to reserve an id number for your module

		// Key text used to identify module (for permissions, menus, etc...)
		$this->rights_class = 'custom_bill';
		...
		$this->description = "Creating Custom Bills";
		// Used only if file README.md and README-LL.md not found.
		$this->descriptionlong = "This is a Module for creating bills from external requests.";

		// Author
		$this->editor_name = 'Maximilian Mörth';
		$this->editor_url = 'https://www.technikum-wien.at';
		...
		$this->dirs = array("/custom_bill/temp");

		// Config pages. Put here list of php page, stored into mymodule/admin directory, to use to setup module.
		$this->config_page_url = array("setup.php@custom_bill");
		...
		$this->langfiles = array("custom_bill@custom_bill");
		...
		*/
		...
		$this->menu[$r++] = array(
			...
			'mainmenu'=>'custom_bill',
			'leftmenu'=>'',
			'url'=>'/custom_bill/custom_bill.php',
			'langs'=>'custom_bill@custom_bill', // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>1000 + $r,
			'enabled'=>'$conf->custom_bill->enabled', // Define condition to show or hide menu entry. Use '$conf->mymodule->enabled' if entry must be visible if module is enabled.
		...
		$this->menu[$r++]=array(
			'fk_menu'=>'fk_mainmenu=custom_bill',	    // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
			'type'=>'left',			                // This is a Left menu entry
			'titre'=>'Settings',
			'mainmenu'=>'custom_bill',
			'leftmenu'=>'custom_bill_myobject_new',
			'url'=>'/custom_bill/graphic.php',
			'langs'=>'custom_bill@custom_bill',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>1000+$r,
			'enabled'=>'$conf->custom_bill->enabled',  // Define condition to show or hide menu entry. Use '$conf->mymodule->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			'perms'=>'$user->rights->custom_bill->myobject->write',			                // Use 'perms'=>'$user->rights->mymodule->level1->level2' if you want your menu with a permission rules
			'target'=>'',
			'user'=>2,				                // 0=Menu for internal users, 1=external users, 2=both
		);
	...
	public function init($options = '')
	{
		...
		$result = $this->_load_tables('/custom_bill/sql/');
		if ($result < 0) {
		...
		// Document templates
		$moduledir = dol_sanitizeFileName('custom_bill');
		$myTmpObjects = array();
